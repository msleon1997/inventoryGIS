...
// Verificar si se proporciona un ID y obtener los datos del proveedor
$proveedor = [];
if (isset($_GET['id'])) {
    $id = $_GET['id'];
    $proveedores = new Proveedores();
    $proveedor = $proveedores->obtenerProveedores($id);
...
if ($_SERVER["REQUEST_METHOD"] == "POST") {
    ...
    $proveedores = new Proveedores();
    $respuesta = $proveedores->crearProveedor($datos);
}
?>
    ...
    <div class="container mt-4">
        <div class="card card-outline card-primary">
            <div class="card-header">
                <h3 class="card-title"><?php echo isset($_GET['id']) ? 'Editar proveedor' : 'Crear Nuevo proveedor'; ?></h3>
            </div>
            <div class="card-body">
                <form id="proveedors_frm" method="post" action="">
                    <div class="row">
                        <div class="form-group col-md-6">
                            ...
                            <input type="text" name="PROV_Identificacion" id="PROV_Identificacion" class="form-control" value="<?php echo isset($proveedor['PROV_Identificacion']) ? $proveedor['PROV_Identificacion'] : ''; ?>" required>
                        </div>
                        <div class="form-group col-md-6">
                            <label for="PROV_persona" class="control-label">Nombre Completo</label>
                            <input type="text" name="PROV_persona" id="PROV_persona" class="form-control" value="<?php echo isset($proveedor['PROV_persona']) ? $proveedor['PROV_persona'] : ''; ?>" required>
                        </div>
                        <div class="form-group col-md-6">
                            <label for="PROV_nombre_empresa" class="control-label">Empresa</label>
                            <input type="text" name="PROV_nombre_empresa" id="PROV_nombre_empresa" class="form-control" value="<?php echo isset($proveedor['PROV_nombre_empresa']) ? $proveedor['PROV_nombre_empresa'] : ''; ?>" required>
                        </div>
                        <div class="form-group col-md-6">
                            <label for="PROV_telefono" class="control-label">Teléfono</label>
                            <input type="tel" name="PROV_telefono" id="PROV_telefono" class="form-control" value="<?php echo isset($proveedor['PROV_telefono']) ? $proveedor['PROV_telefono'] : ''; ?>" required>
                        </div>
                        <div class="form-group col-md-6">
                            <label for="PROV_email" class="control-label">Correo</label>
                            <input type="email" name="PROV_email" id="PROV_email" class="form-control" value="<?php echo isset($proveedor['PROV_email']) ? $proveedor['PROV_email'] : ''; ?>" required>
                        </div>
                        <div class="form-group col-md-6">
                            <label for="PROV_pagina_web" class="control-label">Página Web</label>
                            <input type="url" name="PROV_pagina_web" id="PROV_pagina_web" class="form-control" value="<?php echo isset($proveedor['PROV_pagina_web']) ? $proveedor['PROV_pagina_web'] : ''; ?>">
                        </div>
                       
                        <div class="form-group col-md-6">
                            <label for="PM_Provincia" class="control-label">Provincia</label>
                            <select name="PM_Provincia" id="PM_Provincia" class="form-control" required>
                                <option value="">Seleccione una provincia</option>
                                <?php foreach ($provincias_y_ciudades as $provincia => $ciudades) : ?>
                                    <option value="<?php echo $provincia; ?>" <?php echo isset($proveedor['PM_Provincia']) && $proveedor['PM_Provincia'] == $provincia ? 'selected' : ''; ?>><?php echo $provincia; ?></option>
                                <?php endforeach; ?>
                            </select>
                        </div>
                        <div class="form-group col-md-6">
                            <label for="PM_Ciudad" class="control-label">Ciudad</label>
                            <select name="PM_Ciudad" id="PM_Ciudad" class="form-control" required>
                                <option value="">Seleccione una ciudad</option>
                                <?php if (isset($proveedor['PM_Provincia'])) : ?>
                                    <?php foreach ($provincias_y_ciudades[$proveedor['PM_Provincia']] as $ciudad) : ?>
                                        <option value="<?php echo $ciudad; ?>" <?php echo isset($proveedor['PM_Ciudad']) && $proveedor['PM_Ciudad'] == $ciudad ? 'selected' : ''; ?>><?php echo $ciudad; ?></option>
                                    <?php endforeach; ?>
                                <?php endif; ?>
                            </select>
                        </div>
                        <div class="form-group col-md-6">
                            ...
                            <input type="text" name="PROV_direccion" id="PROV_direccion" class="form-control" value="<?php echo isset($proveedor['PROV_direccion']) ? $proveedor['PROV_direccion'] : ''; ?>" required>
                        </div>
                        <div class="form-group col-md-6">
                            <label for="notas" class="control-label">Notas</label>
                            <textarea name="notas" id="notas" class="form-control"><?php echo isset($proveedor['notas']) ? $proveedor['notas'] : ''; ?></textarea>
                        </div> 
                        <div class="form-group col-md-6">
                            <label for="estado_proveedor" class="control-label">Estado</label>
                            <select name="estado_proveedor" id="estado_proveedor" class="form-control" required>
                                <option value="1" <?php echo isset($proveedor['estado_proveedor']) && $proveedor['estado_proveedor'] == 2 ? 'selected' : ''; ?>>Activo</option>
                                <option value="0" <?php echo isset($proveedor['estado_proveedor']) && $proveedor['estado_proveedor'] == 1 ? 'selected' : ''; ?>>Inactivo</option>
                            </select>
                        </div>
                        <input type="hidden" name="PM_Latitud" id="PM_Latitud" value="<?php echo isset($proveedor['PM_Latitud']) ? $proveedor['PM_Latitud'] : ''; ?>">
                        <input type="hidden" name="PM_Longitud" id="PM_Longitud" value="<?php echo isset($proveedor['PM_Longitud']) ? $proveedor['PM_Longitud'] : ''; ?>">

                    </div>
                    <input type="hidden" name="users_id" value="<?php echo $_settings->userdata('id'); ?>">
                    <div id="mapa"></div>
                    <div class="form-group">
                        <button class="btn btn-flat btn-primary btn-sm" id="geocodeButton" type="submit">Registrar Proveedor</button>
    ...
    document.getElementById('PM_Ciudad').addEventListener('change', function() {
        ...
        if (coordenadas[ciudad]) {
            ...
            centrarMapaEnCoordenadas(latlng[0], latlng[1]);
        }
    });

    // Si se está editando un proveedor y ya hay coordenadas guardadas, mostrar el marcador
    var latitud = document.getElementById('PM_Latitud').value;
    var longitud = document.getElementById('PM_Longitud').value;

    if (latitud && longitud) {
        centrarMapaEnCoordenadas(latitud, longitud);
    }
});
</script>
